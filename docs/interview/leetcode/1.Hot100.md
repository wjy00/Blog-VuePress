...
[LeetCode 链接](https://leetcode-cn.com/problems/majority-element/)
...
使用 Map 保存元素，每次遍历检查数量是否大于 `n/2`，大于则返回该值。
...
var majorityElement = function(nums) {
  ...
  for(let i of nums) {
    const num = map.get(i)
    if(num + 1 > len / 2) return i
    map.set(i, (num ? num : 0) + 1)
  }
};
```
...
排序法后，由于最多的数的数量比一半还多，所以在数组最中间的数肯定是该数。
...
[LeetCode 链接](https://leetcode-cn.com/problems/house-robber/)
...
+ `dp[i]` 表示到第 `i` 项为止，所能偷的最高金额
+ dp[i] 的值为：`dp[i] = Math.max(dp[i - 1], dp[i - 2] + nums[i])`
...
用递归函数不断传入 `head.next`，直到 `head==null` 或者 `heade.next==null` 
...
[LeetCode 链接](https://leetcode-cn.com/problems/course-schedule/)

![](./assets/207.png)

### 207.2 题解

+ `inDegree[i]` 表示第 i 门课，还有多少门先修课程没有上
+ `graph` 存放当前课程的后续课程1
+ `queue` 存放课程队列
+ 遍历 `queue`，减少 `inDegree` 的值
+ 最后判断 `inDegree` 中是否全为 0
...
**思路：**
快速排序采用分而治之的思想，将待排序的数中取出一个数作为基准数，<u>将比这个数大的数字全部放在它的右边，比这个数字小的或者等于的全部放在它的左边</u>，再对左右区间重复第二步，直到各个区间只有一个数。基准值我选择数组的第一项。

**过程：**

![](https://images2015.cnblogs.com/blog/825641/201702/825641-20170228154948329-520263358.gif)
